...
export function DeploymentDashboard({
  ...
  async function settleExposureChange(
    update: DeploymentExposureUpdate
  ): Promise<void> {
    setDeployments((current) =>
      current.map((deployment) =>
        deployment.serviceName === update.serviceName &&
        (update.namespace === undefined ||
          deployment.namespace === update.namespace)
          ? {
              ...deployment,
              exposure: update.exposure,
              tailscaleOnly: update.tailscaleOnly,
            }
          : deployment
      )
    )
          ...
          {loading && deployments.length === 0 ? (
            ...
            deployments.map((deployment) => (
              <DeploymentCard
                ...
                key={deployment.host}
                onExposureChanged={settleExposureChange}
              />
            ))
          )}
        </section>
