...
| Status/global | --status-global | #0891b2 | #67e8f9 | Public internet route border |
| Status/global-accent | --status-global-accent | #d97706 | #fbbf24 | Public internet route highlight |

### Rules
- Accent is reserved for actions, focus, and successful routes.
- Warning is used only for dry-run or pending deployment states.
- Public internet exposure uses the info/global tokens and must be labeled in text, not color alone.
...
### Internet Route Frame
- Structure: restrained global border treatment around an internet-exposed deployment card plus a short text label.
- Variants: internet only.
- Spacing: follows Deployment Card spacing.
- States: default, hover.
- Accessibility: visible `Internet network` label accompanies the border.
- Motion: 120ms border/background transition only.
