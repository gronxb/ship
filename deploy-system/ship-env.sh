...
env_cloudflare_account_id_set=${CLOUDFLARE_ACCOUNT_ID+x}
env_cloudflare_account_id=${CLOUDFLARE_ACCOUNT_ID-}
env_cf_account_id_set=${CF_ACCOUNT_ID+x}
...
env_cloudflare_tunnel_id_set=${CLOUDFLARE_TUNNEL_ID+x}
env_cloudflare_tunnel_id=${CLOUDFLARE_TUNNEL_ID-}
env_ship_cloudflare_tunnel_name_set=${SHIP_CLOUDFLARE_TUNNEL_NAME+x}
env_ship_cloudflare_tunnel_name=${SHIP_CLOUDFLARE_TUNNEL_NAME-}
...
if [ -n "$env_cloudflare_account_id_set" ]; then
  CLOUDFLARE_ACCOUNT_ID=$env_cloudflare_account_id
fi
if [ -n "$env_cf_account_id_set" ]; then
...
if [ -n "$env_cloudflare_tunnel_id_set" ]; then
  CLOUDFLARE_TUNNEL_ID=$env_cloudflare_tunnel_id
fi
if [ -n "$env_ship_cloudflare_tunnel_name_set" ]; then
  SHIP_CLOUDFLARE_TUNNEL_NAME=$env_ship_cloudflare_tunnel_name
fi
...
if [ -n "${SHIP_CLOUDFLARE_TUNNEL_NAME:-}" ]; then
  export SHIP_CLOUDFLARE_TUNNEL_NAME
fi
